...
# Application instance — must exist before any route decorators
app = FastAPI(title="PKI + TOTP Microservice")

# Load student private key (raises FileNotFoundError if missing)
...
try:
    ...
    # If startup can't load key, raise so server logs it clearly.
...
def decrypt_seed(payload: EncryptedSeed):
    try:
        ...
        # Accept either ASCII hex string or raw bytes
        try:
            seed_text = plain.decode().strip()
            # If decode succeeds but it's not 64-hex, fall back to hex()
            if not is_valid_hex_seed(seed_text):
                seed_text = plain.hex()
        except UnicodeDecodeError:
            # binary form -> convert to hex
            seed_text = plain.hex()

        if not is_valid_hex_seed(seed_text):
            raise ValueError("Decrypted seed invalid (not 64 hex chars)")
